...
**Linux:**
- User config: `~/.config/pwshpy/config.toml`
- App config: `/etc/xdg/pwshpy/config.toml`
...
pwshpy config --format json

# Show specific section only
pwshpy config --section lib_log_rich

# Load configuration for a specific profile
pwshpy config --profile production

# Combine options
pwshpy config --profile staging --format json --section lib_log_rich
```
...
pwshpy config-deploy --target user

# Deploy to system-wide location (requires privileges)
sudo pwshpy config-deploy --target app

# Deploy host-specific configuration
sudo pwshpy config-deploy --target host

# Deploy to multiple locations at once
pwshpy config-deploy --target user --target host

# Overwrite existing configuration
pwshpy config-deploy --target user --force

# Deploy to a specific profile directory
pwshpy config-deploy --target user --profile production

# Deploy production profile and overwrite if exists
pwshpy config-deploy --target user --profile production --force
```
...
sudo -u serviceaccount pwshpy config-deploy --target user

# Deploy with a specific profile
sudo -u serviceaccount pwshpy config-deploy --target user --profile production

# The config will be created at that user's home directory:
# /home/serviceaccount/.config/pwshpy/config.toml
```
...
sudo -u myservice pwshpy config-deploy --target user
...
pwshpy config-deploy --target user --no-permissions

# Override directory mode (octal)
pwshpy config-deploy --target user --dir-mode 750

# Override file mode (octal)
pwshpy config-deploy --target user --file-mode 640

# Both overrides together
pwshpy config-deploy --target user --dir-mode 750 --file-mode 640

# Octal formats: both "750" and "0o750" are accepted
pwshpy config-deploy --target user --dir-mode 0o750
```
...
pwshpy config-generate-examples --destination ./examples --force

# Generate examples in current directory
pwshpy config-generate-examples --destination .
```
...
pwshpy --set lib_log_rich.console_level=DEBUG --set lib_log_rich.console_format_preset=short config

# Override nested values
pwshpy --set lib_log_rich.payload_limits.truncate_message=2048 config

# Override with JSON values (use single quotes around the value)
pwshpy --set lib_log_rich.console_level='"DEBUG"' config

# Combine with profile
pwshpy --profile production --set lib_log_rich.console_level=DEBUG config
```
...
**Without profile:**
- `~/.config/pwshpy/config.toml`

**With profile `production`:**
- `~/.config/pwshpy/profile/production/config.toml`
...
```bash
LOG_CONSOLE_LEVEL=DEBUG pwshpy processes
...
```bash
PWSHPY___LIB_LOG_RICH__CONSOLE_LEVEL=DEBUG pwshpy processes
PWSHPY___LIB_LOG_RICH__CONSOLE_STREAM=stderr pwshpy info
```
...
pwshpy --env-file /opt/myapp/config/.env config
pwshpy --env-file ./environments/staging.env config
```
