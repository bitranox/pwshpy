...
A thin front end over the ``ps`` facade (Tier A / psutil), mirroring
...
@click.command("connections", context_settings=CLICK_CONTEXT_SETTINGS)
...
def cli_connections(ctx: click.Context, as_json: bool, as_jsonl: bool, limit: int | None) -> None:
    """List open network connections as typed records (Tier A, native).
