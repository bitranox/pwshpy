...
@click.command("resolve", context_settings=CLICK_CONTEXT_SETTINGS)
@argument("name")
...
def cli_resolve(ctx: click.Context, name: str, timeout: float | None, as_json: bool, as_jsonl: bool) -> None:
    """Resolve NAME to its addresses as typed records (Tier A, native).
    ...
    with lib_log_rich.runtime.bind(job_id="cli-resolve", extra={"command": "resolve"}):
        ...
        emit(ps.resolve(name, timeout=timeout), fmt)
