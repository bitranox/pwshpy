...
A thin front end over the ``ps`` facade (Tier A / psutil): it parses options,
...
@click.command("processes", context_settings=CLICK_CONTEXT_SETTINGS)
...
def cli_processes(ctx: click.Context, as_json: bool, as_jsonl: bool, limit: int | None) -> None:
    """List running processes as typed records (Tier A, native — no pwsh subprocess).
