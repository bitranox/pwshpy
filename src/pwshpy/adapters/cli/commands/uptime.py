...
A thin front end over the ``ps`` facade (Tier A / psutil).  Emits a single
...
@click.command("uptime", context_settings=CLICK_CONTEXT_SETTINGS)
...
def cli_uptime(ctx: click.Context, as_json: bool, as_jsonl: bool) -> None:
    """Show system boot time and elapsed uptime as a typed record (Tier A, native).
