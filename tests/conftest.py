...
_COVERAGE_BASENAME = ".coverage.pwshpy"
...
def config_factory() -> Callable[[dict[str, Any]], Config]:
    ...
    Example:
        def test_config_section(config_factory: Callable[[dict[str, Any]], Config]) -> None:
...
def source_info_factory() -> Callable[[str, str, str | None], SourceInfo]:
    ...
    Example:
        def test_provenance(source_info_factory: Callable[..., SourceInfo]) -> None:
            info = source_info_factory("section.key", "user", "/home/user/.config/...")
