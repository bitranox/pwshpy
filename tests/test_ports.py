...
from pwshpy.adapters.memory import (
...
if TYPE_CHECKING:
    from pwshpy.application.ports import (
        GetConfig,
...
def test_build_production_returns_fully_populated_app_services() -> None:
    ...
    from pwshpy.composition import AppServices, build_production
...
# The ``ps`` field is a facade object, not a callable port.
_PORT_FIELDS = ("get_config", "get_default_config_path", "deploy_configuration", "display_config", "init_logging")


@pytest.mark.os_agnostic
def test_build_production_services_are_callable() -> None:
    """All port services from build_production() must be callable."""
    from pwshpy.composition import build_production
...
def test_build_testing_services_are_callable() -> None:
    """All port services from build_testing() must be callable."""
...
@pytest.mark.os_agnostic
def test_build_production_wires_ps_facade() -> None:
    """build_production() exposes a usable ps facade."""
    from pwshpy.composition import Ps, build_production

    assert isinstance(build_production().ps, Ps)
